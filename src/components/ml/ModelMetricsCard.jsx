...
const metrics = [
  { id: 'tss', name: 'TSS', value: 0.74, label: 'True Skill Score', desc: 'Measures how well the forecast separates events from non-events. > 0.5 indicates skill above random. Primary metric.' },
  { id: 'pod', name: 'POD', value: 0.81, label: 'Probability of Detection', desc: 'The fraction of observed flares that were correctly forecasted (Hit Rate).' },
  { id: 'far', name: 'FAR', value: 0.28, label: 'False Alarm Ratio', desc: 'The fraction of forecasted flares that did not actually occur. Lower is better.' },
  { id: 'hss', name: 'HSS', value: 0.68, label: 'Heidke Skill Score', desc: 'Measures fractional improvement of the forecast over the standard random forecast.' }
];
...
export function ModelMetricsCard() {
  ...
  return (
    <Card className="flex flex-col" p={4} title="MODEL METRICS (HELD-OUT SET)">
      ...
      <div className="mt-4 p-2 bg-[#0A1929] border border-border-subtle rounded-sm flex gap-2 items-start">
        ...
        <span className="font-mono text-[9px] text-text-secondary uppercase tracking-wider leading-relaxed">
          Evaluated on 1000-sample held-out test set.<br/>
          TSS &gt; 0.5 indicates skill above random.
        </span>
      </div>
    </Card>
  );
